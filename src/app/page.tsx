'use client'

import { PropertyMapContainer } from '@/components/PropertyMapContainer'
import { N8nChat } from '@/components/N8nChat'
import { useState } from 'react'
import Link from 'next/link'

export default function Home() {
  const [refreshProperties] = useState(0)

  return (
    <main className="h-screen relative overflow-hidden">
      {/* Full Screen Map - extends to bottom */}
      <div className="absolute inset-0">
        <PropertyMapContainer refreshTrigger={refreshProperties} showSidebar={false} />
      </div>

      {/* Admin Link - top right */}
      <Link
        href="/admin"
        className="absolute top-4 right-4 z-20 bg-white hover:bg-gray-50 text-gray-700 font-semibold p-3 rounded-full shadow-lg transition-all duration-200 flex items-center justify-center"
        title="Admin"
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
      </Link>

      {/* N8n Chat with Input Bar - floats on top */}
      <N8nChat chatUrl="https://jpinnmobiliar.app.n8n.cloud/webhook/499666c3-d807-4bb7-8195-43932f64a91f/chat" />
    </main>
  )
}
